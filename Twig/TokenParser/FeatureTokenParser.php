...
class FeatureTokenParser extends \Twig_TokenParser
{
    ...
    public function parse(\Twig_Token $token)
    {
        ...
        $body = $this->parser->subparse(array($this, 'decideFeatureFork'));
        ...
        while (!$end) {
            switch ($this->parser->getStream()->next()->getValue()) {
                case 'else':
                    $stream->expect(\Twig_Token::BLOCK_END_TYPE);
                    $else = $this->parser->subparse(array($this, 'decideFeatureEnd'));
                    break;
        ...
        return new FeatureNode($name, $parent, $body, $else, $lineno, $this->getTag());
    }

    public function decideFeatureFork(\Twig_Token $token)
    {
        return $token->test(array('else', 'endfeature'));
    }

    public function decideFeatureEnd(\Twig_Token $token)
    {
        return $token->test(array('endfeature'));
    }
